...
const AdminDashboard = () => {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      setLoading(true);
      try {
        const res = await axios.get("http://localhost:4000/api/admin/dashboard/counts",{
          headers:{
    ...
    <>
      <div className='p-6'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6'>
          ...
          <Card title="Total Doctors" value={counts.totalDoctors}/>
          <Card title="Total Patients" value={counts.totalPatients}/>
        </div>
      </div>
    </>
  )
