...
const AllBookedAppointments = () => {
  ...
  const fetchBookedAppointments = async() =>{
    try {
      const res = await axios.get("http://localhost:4000/api/doctor/appointments/booked/all",{
        headers:{
  ...
  if(loading){
    return <p className='text-center mt-5'>Loading Booked appointments...</p>
  }

  if(!appointments.length){
    return <p className='p-4 text-gray-600'>No Booked appointments found</p>
  }
  return (
    <>
      <div className='p-4'>
                  ...
                  <div>
                    {appt.status === "BOOKED" && (
                      <div className='mt-4'>
                        ...
                        receiverId={appt.patient.userId}/>
                      </div>
                    )}
                  </div>
